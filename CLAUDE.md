# Programming Fundamentals Lab — Project Guide
...
├── slides/                    # one independent deck per lab
...
│   ├── labs.json              # ⭐ single source of truth for the lab structure
│   ├── labs.ts                # navigation logic between decks
...
### Why one deck per lab

Every lab is a separate Slidev app rather than one deck stitched together with
`src:` includes. Slide numbering is then local to a lab (adding a slide to lab 2
cannot shift the links in lab 7), each dev server only reloads its own file, and
a lab can be presented on its own.
...
they are paths under a shared base. `common/labs.ts` (`deckUrl`) is the only
place that knows the difference.

### `common/labs.json` is the single source of truth ⭐

Lab number, slug, port, module, icon, title and description all live in this one
file. `scripts/*.mjs` read it at build time; `common/labs.ts` reads it at
runtime. The hub grid, the prev/next `<DeckNav>` and the generated deck titles
are all derived from it.

**Never hardcode** a lab title, port or neighbour link in a deck — edit the
registry instead.
...
- ❌ `# Lab 5` / `## Tablouri`
- ❌ `# Lab 5: Tablouri`
- ❌ `# Laboratorul 5 — Tablouri`
- ✅ `# Tablouri (Arrays) în C`
- ✅ `# Problema Stocării Multiplelor Valori`

No lab number, no `Lab N` prefix, no numbering of any kind in the visible
heading — the number already appears in the hub grid and in the navigation.

The numbered form (`Lab 5: Tablouri`) belongs **only** in `common/labs.json`
...
```yaml
---
title: 'FP · Lab 5: Tablouri'     # generated from common/labs.json
info: '…'                          # generated from common/labs.json
...
|---|---|
| `<LabGrid />` | Grid of all labs, grouped by module (used by the hub) |
| `<LabCard slug="05-arrays" />` | A single card in that grid |
| `<DeckNav />` | End-of-deck navigation: previous lab / hub / next lab |
| `<HomeButton />` | 🏠 back to the hub; rendered on every slide via `global-bottom.vue` |
| `<GithubLink />` | Repository link icon (repo URL comes from `labs.json`) |
...
with a collection prefix: `<LabGrid/>` becomes the icon `la/b-grid` (`la` = Line
...
1. Add the entry to `common/labs.json` — number, slug, port, module, icon, title,
...
node scripts/dev.mjs hub 5 6    # only the hub and labs 5 and 6
pnpm dev:hub                    # a single deck
pnpm dev:05-arrays
```

Ports: `3030` hub, `3030 + lab number` for labs. Cross-deck links point at
...
The lab currently being taught is set in `common/theme/config.ts` (`currentLab`,
a slug) — earlier labs show ✅ in the hub grid, this one ▶️.
...
The hub builds to `dist/`, each lab to `dist/<slug>/`. `scripts/fix-spa-routing.mjs`
...
**Renaming a deck** touches more than the directory: `common/labs.json` (`slug`),
the deck's `package.json` (`name`), the root `package.json` (`dev:*` script),
`common/theme/config.ts` (`currentLab`), `README.md`, then `pnpm install` and
`pnpm sync-headmatter`.
...
- [ ] Slide titles carry **only the title** — no `Lab N`, no numbering
- [ ] `color: stone-light` and `align: c` on every content slide
- [ ] No `v-click` / `v-clicks`
- [ ] Runnable C examples compile clean and are complete programs
- [ ] Lab metadata changed in `common/labs.json`, not in the deck
